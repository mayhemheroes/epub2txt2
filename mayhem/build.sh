...
: "${MAYHEM_JOBS:=$(nproc)}"
# Debug-info contract (§6.2 item 10): Mayhem's triage cannot read DWARF >= 4, and clang-19's plain
# `-g` (which SANITIZER_FLAGS carries) emits DWARF-5 — so pin the debug version explicitly. Threaded
# into EVERY fuzz/harness/standalone compile+link (and the instrumented project objects) below so the
# target binary and its -standalone reproducer carry .debug_info at DWARF-3. `-gdwarf-3` comes LAST so
# it overrides the bare `-g`. The base image exports DEBUG_FLAGS when present; default it here so the
# build is correct on bases that don't, and so an empty SANITIZER_FLAGS (no `-g`) still gets DWARF-3.
...
export SANITIZER_FLAGS CC LIB_FUZZING_ENGINE MAYHEM_JOBS DEBUG_FLAGS
...
make -j"$MAYHEM_JOBS" CC="$CC" EXTRA_CFLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS" \
...
HCFLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS -I src -DAPPNAME=\"epub2txt\" -DVERSION=\"fuzz\""
